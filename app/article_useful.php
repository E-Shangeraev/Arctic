...
<body>
    ...
    <main class="article-page">
        <section class="article scrollify" data-section="1">
            <div class="article__wrapper wrapper wrapper--column">
              <article style="width: 60%">
                  <?php
                      $article = mysqli_query($connection, "SELECT * FROM `useful_items` WHERE `id` = " . (int) $_GET['id']);
                      $categories = mysqli_query($connection, "SELECT * FROM `useful`");
                  ...
                  <?php if ( mysqli_num_rows($article) <= 0): ?>
                      ...
                      <?php else: 
                          mysqli_query($connection, "UPDATE `useful_items` WHERE `id` = " . (int) $art['id']);
                          $art_cat = false;
                      ...
                      ?>
                      <h2 class="article__title" style="margin-bottom: 0"><?= $art['name']; ?></h2>
                      ...
                      <img src="data:image/png;base64, <?= base64_encode($art["image"]) ?>" class="article__image" alt="<?= $art['name']; ?>"/>
